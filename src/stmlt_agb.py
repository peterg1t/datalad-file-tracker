...
def match_graphs(provenance_ds_path, gdb_abstract, ds_branch):
    ...
    branch.checkout()
    node_mapping = import_export.translation_file_process(
        f"{provenance_ds_path}/tf.csv"
    )
...
def run_pending_nodes(scheduler_instance, provenance_ds_path, gdb_difference, branch):
    ...
    # file names instead of the abstract names. From the nodes we can extract the list
    # of inputs and outputs for the job that is going to run
    node_mapping = import_export.translation_file_process(
        f"{provenance_ds_path}/tf.csv"
    )
    ...
    except ValueError as error_provenance:  # pylint: disable = bare-except
        st.warning(
            f"No provance graph has been matched to this abstract graph, match one first {error_provenance}"  # noqa: E501
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # Added argument parser to parse a file with a workflow from
    # a text file, the text file format will be the following format
    # {inputs}<>{task}<>{outputs}
    parser.add_argument(
        "-a",
        "--agraph",
    ...
    try:
        ...
        st.warning(
            f"""There was a problem in the creation of the graph
             verify that all node names match along the edges {error}"""
        )
        st.stop()
    ...
    if st.sidebar.button("Generate code"):
        code_workflow = import_export.generate_code(gdb)
